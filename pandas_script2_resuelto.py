...
df[ (df['nota1']>=7) & (df['nota2']>=7)] # Doble filtro
...
df.loc['1/21', 'nombre'] = 'Daniel' 

print(df)

promedios = (df["nota1"] + df ["nota2"])/2
...
print(df["nota1"]>=6)
print("Cantidad de alumnos con mayor nota a 6 =", (df["nota1"]>=6).sum())

print("------------")

mayores_6 = (df[df["nota1"]>=6])
print(mayores_6)

print("------------")

aprobados = df[(df["nota1"]>=6) & (df["nota2"]>=7)]
print("Df de aprobados:\n", aprobados)
...
ap_con_menos_de_7_en_2 = df[(df["aprueba"]==True) & (df["nota2"]<=7)]
print(ap_con_menos_de_7_en_2)


print("------------")

menos_que_6 = df[(df["nota1"]<6) | (df["nota2"] <6)]
print(menos_que_6)

print("------------")

df_filtrado = df.drop(df[df['nota1'] == np.nan].index)
print(df_filtrado)
...
cant_por_nota = {}
# df.drop("47/20", inplace=True)
for i in range(1,11):
    cant_por_nota[i] = (df["nota1"]==float(i)).sum()

print(cant_por_nota)

print("------------")

reemplazo_por_0 = df["nota1"].where(df['nota1'] <=4 , 0)
print(reemplazo_por_0)