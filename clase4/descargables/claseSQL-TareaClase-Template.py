...
carpeta = "/home/Estudiante/Documentos/LaboDatos-UBA/clase3/descargables/"
...
consultaSQL = """
                ...
                INTERSECT
                SELECT DISTINCT NroVuelo FROM reserva;
              """
...
dniPersonasDesdeMadrid = dd.query("""
                ...
                FROM reserva AS r
                INNER JOIN vuelosDesdeMadrid AS vuel
                ON vuel.Numero = r.NroVuelo
              """).df()
...
consultaSQL = """
                SELECT Instancia, AVG(Nota) AS Asistieron
                FROM examen
                GROUP BY Instancia
...
consultaSQL = """
                SELECT DISTINCT e1.Nombre, e1.Sexo, e1.Edad
                FROM examen AS e1
              """
...
auxRecuperatorio01 = dd.query(consultaSQL).df()

consultaSQL = """
                SELECT DISTINCT aux.*, Nota as Recuperatorio_02
                FROM auxRecuperatorio01 AS aux
                LEFT OUTER JOIN examen AS e
                ON aux.Nombre=e.Nombre AND Instancia='Recuperatorio-02'
            """
            
auxRecuperatorio02 = dd.query(consultaSQL).df()
...
aux_parcial01=dd.query(consultaSQL).df()
...
aux_parcial02=dd.query(consultaSQL).df()
...
aux_recuperatorio01=dd.query(consultaSQL).df()
...
aux_recuperatorio02=dd.query(consultaSQL).df()
...
aux_union01=dd.query(consultaSQL).df()
...
consultaSQL = """
        ...
        UNION
            SELECT *
            FROM aux_recuperatorio01
            """
            
aux_union02=dd.query(consultaSQL).df()
